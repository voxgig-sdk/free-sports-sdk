...
type Args struct {
	Entity string         `json:"entity" jsonschema:"event | league | player | team"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.FreeSportsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "free-sports_list",
		Description: "List records from FreeSports. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "free-sports_load",
		Description: "Load a single record from FreeSports. " +
...
func runOp(client *sdk.FreeSportsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.FreeSportsSDK, name string) (sdk.FreeSportsEntity, error) {
	switch strings.ToLower(name) {
	case "event":
		return client.Event(nil), nil
	case "league":
		return client.League(nil), nil
	case "player":
