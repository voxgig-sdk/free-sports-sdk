...
[MCP](https://modelcontextprotocol.io) server exposing the FreeSports SDK as
two agent tools — `free-sports_list` and `free-sports_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-sports-mcp)
make build

# 2. Provide credentials via the environment
export FREE_SPORTS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-sports \
  -- /absolute/path/to/free-sports-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-sports-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-sports_list: first page of records
{ "entity": "event" }
{ "entity": "event", "query": { } }

// free-sports_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-sports-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-sports \
     -- "$PWD"/dist/*/free-sports-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-sports_list` and `free-sports_load` tools now appear
   in new sessions. Ask the agent to *"list event using free-sports"*
   and it calls `free-sports_list` with `{"entity":"event"}`.
...
```sh
export FREE_SPORTS_APIKEY=sk_live_xxx            # API key
export FREE_SPORTS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `free-sports_list` tool
...
### Call the `free-sports_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-sports_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-sports_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_SPORTS_APIKEY` | API key sent with every request. |
| `FREE_SPORTS_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

event | league | player | team

### Smoke test via HTTP (raw JSON-RPC)

```sh
./free-sports-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-sports_load","arguments":{"entity":"event","query":{"id":1}}}}'
```
