...
class FreeSportsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeSportsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeSportsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeSportsTestRunner.getenv("FREESPORTS_TEST_LIVE")
        override = FreeSportsTestRunner.getenv("FREESPORTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeSportsTestRunner.getenv(key)
        ...
        explain = FreeSportsTestRunner.getenv("FREESPORTS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREESPORTS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeSportsTestRunner._test_control is not None:
            return FreeSportsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeSportsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeSportsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeSportsTestRunner.load_test_control()
...
def load_env_local():
    FreeSportsTestRunner.load_env_local()


def env_override(m):
    return FreeSportsTestRunner.env_override(m)


def entity_list_to_data(lst):
    return FreeSportsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeSportsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeSportsTestRunner.load_test_control()


def live_delay_ms():
    return FreeSportsTestRunner.live_delay_ms()
