...
class TestLeagueDirect:

    def test_should_direct_list_league(self):
        setup = _league_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-league", "live" if setup["live"] else "unit")
        if _skip:
...
def _league_direct_setup(mockres):
