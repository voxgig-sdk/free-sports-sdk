...
class TestPlayerDirect:
    ...
    def test_should_direct_list_player(self):
        setup = _player_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-player", "live" if setup["live"] else "unit")
        if _skip:
