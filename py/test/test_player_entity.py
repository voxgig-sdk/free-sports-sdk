...
class TestPlayerEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Player(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Player(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREESPORTS_TEST_PLAYER_ENTID JSON to run live")
        client = setup["client"]
        ...
        player_ref01_list_result = player_ref01_ent.list(player_ref01_match, None)
        assert isinstance(player_ref01_list_result, list)
...
def _player_basic_setup(extra):
    ...
    idmap = vs.transform(
        ["player01", "player02", "player03", "api_key01"],
        {
            "`$PACK`": ["", {
                "`$KEY`": "`$COPY`",
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREESPORTS_TEST_PLAYER_ENTID"))
