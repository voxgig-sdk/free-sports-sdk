...
The Python SDK for the FreeSports API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Event(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.FreeSports(None).load(
    {"id": "test01"}, None
)
