...
AQL-driven command-line client **and** interactive REPL for the FreeSports
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-sports-cli)
make build

# 2. See usage (words, entities, env vars)
./free-sports-cli --help
...
./free-sports-cli list event
./free-sports-cli list league

# 5. Override the API base URL for a single call
FREE_SPORTS_BASE=https://api.example.com ./free-sports-cli list event

# 6. No arguments -> interactive REPL
./free-sports-cli
free-sports> list event
free-sports> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-sports-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-sports-cli list event
   ```
...
Run with no arguments to open a REPL (prompt `free-sports>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `event`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-sports-cli`. |
...
(`./free-sports-cli <expr>`) and interactively (the REPL), and expressions compose the
