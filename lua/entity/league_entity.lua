...
local LeagueEntity = {}
LeagueEntity.__index = LeagueEntity


function LeagueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LeagueEntity)
  self._name = "league"
  self._client = client
...
function LeagueEntity:get_name()
  return self._name
end


function LeagueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LeagueEntity.new(self._client, opts)
end


function LeagueEntity:data_set(args)
  if args ~= nil then
...
function LeagueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LeagueEntity:match_set(args)
  if args ~= nil then
...
function LeagueEntity:match_get()
...
function LeagueEntity:stream(action, args, callopts)
...
---@param reqmatch LeagueListMatch
---@param ctrl? table
---@return League[]
---@return string? err
function LeagueEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function LeagueEntity:_run_op(ctx, post_done)
  local utility = self._utility
