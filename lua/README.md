...
The Lua SDK for the FreeSports API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Event(nil):list(nil, nil)
...
local result, err = client:FreeSports(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── free-sports_sdk.lua    -- Main SDK module
...
The main module (`free-sports_sdk`) exports the SDK constructor
