...
{
    private static function utility(): FreeSportsUtility
    {
        return new FreeSportsUtility();
    }
    ...
    private static function ctx(array $over = []): FreeSportsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreeSportsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreeSportsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreeSportsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreeSportsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreeSportsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreeSportsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreeSportsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreeSportsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreeSportsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeSportsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreeSportsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreeSportsResponse([]),
            'result' => new FreeSportsResult([]),
        ]);
        [, $err] = FreeSportsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FreeSportsSpec([]),
            'response' => null,
            'result' => new FreeSportsResult([]),
        ]);
        [, $err] = FreeSportsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreeSportsSpec(['step' => 's']),
            'response' => new FreeSportsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreeSportsResult([]),
        ]);
        [, $err] = FreeSportsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeSportsSpec(['step' => 's']),
            'response' => new FreeSportsResponse(self::resp(200, ['v' => 1])),
            'result' => new FreeSportsResult([]),
        ]);
        [, $err] = FreeSportsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeSportsSpec(['step' => 's']),
            'response' => new FreeSportsResponse(self::resp(200, ['v' => 2])),
            'result' => new FreeSportsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreeSportsMakeResponse::call($ctx);
    ...
    {
        $preset = new FreeSportsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreeSportsSpec([]),
            'response' => new FreeSportsResponse([]),
            'result' => new FreeSportsResult([]),
        ...
        [$got, $err] = FreeSportsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreeSportsResult([])]);
        [, $err] = FreeSportsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreeSportsSpec([]), 'result' => null]);
        [, $err] = FreeSportsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeSportsSpec(['step' => 's']),
            'result' => new FreeSportsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreeSportsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeSportsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeSportsSpec(['step' => 's']),
            'result' => new FreeSportsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreeSportsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeSportsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeSportsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreeSportsSpec([]),
            'result' => new FreeSportsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreeSportsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeSportsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreeSportsError('boom', 'boom');
        $utility->fetcher = function (FreeSportsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeSportsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreeSportsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreeSportsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreeSportsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreeSportsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FreeSportsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreeSportsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeSportsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreeSportsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreeSportsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreeSportsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreeSportsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreeSportsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreeSportsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreeSportsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreeSportsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreeSportsResult(['ok' => false])]);
        $this->expectException(FreeSportsError::class);
        FreeSportsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreeSportsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreeSportsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FreeSportsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreeSportsBaseFeature();
        $b = new FreeSportsBaseFeature();
        FreeSportsFeatureAdd::call($ctx, $a);
        FreeSportsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreeSportsBaseFeature
    {
        $f = new FreeSportsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreeSportsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreeSportsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreeSportsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeSportsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FreeSportsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreeSportsResponse(['headers' => null]),
            'result' => new FreeSportsResult([]),
        ]);
        FreeSportsResultHeaders::call($ctx);
