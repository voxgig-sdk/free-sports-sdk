...
//     (\`new FreeSportsSDK(...)\` / \`FreeSportsSDK.test(...)\` become
//     \`FreeSportsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"event":{"test01":{"id":"test01"}},"league":{"test01":{"id":"test01"}},"player":{"test01":{"id":"test01"}},"team":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeSportsSDK(...)\` and any
// \`FreeSportsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeSportsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeSportsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
