...
class PlayerEntity
  ...
  def data_set(args)
    if args
      @_data = FreeSportsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreeSportsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [PlayerListMatch, Hash, nil] match filter (any subset of
  #   Player fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Player>, Array] the matching Player items; raises FreeSportsError on failure
