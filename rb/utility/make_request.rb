...
module FreeSportsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeSportsResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeSportsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeSportsResponse.new(fetched)
    else
