...
class LeagueDirectTest < Minitest::Test
  def test_direct_list_league
    setup = league_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-league", setup[:live] ? "live" : "unit")
    if _should_skip
...
def league_direct_setup(mockres)
