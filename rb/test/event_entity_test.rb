...
class EventEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_EVENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def event_basic_setup(extra)
  ...
  idmap = Vs.transform(
    ["event01", "event02", "event03", "api_key01"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["FREESPORTS_TEST_EVENT_ENTID"]
