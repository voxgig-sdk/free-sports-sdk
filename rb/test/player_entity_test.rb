...
class PlayerEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FreeSportsSDK.test(seed, nil)
    seen = base.Player(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Player(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_PLAYER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def player_basic_setup(extra)
  ...
  entid_env_raw = ENV["FREESPORTS_TEST_PLAYER_ENTID"]
