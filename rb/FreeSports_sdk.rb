...
class FreeSportsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeSportsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeSportsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeSportsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeSportsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = FreeSportsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeSportsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def direct(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FreeSportsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
