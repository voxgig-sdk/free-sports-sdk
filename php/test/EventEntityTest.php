...
class EventEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_EVENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function event_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["event01", "event02", "event03", "api_key01"] as $k) {
    ...
    $entid_env_raw = getenv("FREESPORTS_TEST_EVENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FREESPORTS_TEST_EVENT_ENTID"]);
