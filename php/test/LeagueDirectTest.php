...
class LeagueDirectTest extends TestCase
{
    public function test_direct_list_league(): void
    {
        $setup = league_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-league", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function league_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new FreeSportsSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
