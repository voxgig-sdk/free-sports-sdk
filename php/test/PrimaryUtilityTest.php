...
// FreeSports SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        FreeSportsSDK $client,
        FreeSportsUtility $utility
    ): FreeSportsContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new FreeSportsContext($ctxmap, null);
    ...
    private static function fixctx(FreeSportsContext $ctx, FreeSportsSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?FreeSportsError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FreeSportsError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new FreeSportsResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new FreeSportsBaseFeature();
    ...
    {
        $hook_client = FreeSportsSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends FreeSportsBaseFeature {
            public $hook_fn = null;
            public function TestHook(FreeSportsContext $ctx): void
            {
    ...
    {
        $init_client = FreeSportsSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends FreeSportsBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new FreeSportsSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new FreeSportsSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = FreeSportsSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = FreeSportsSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof FreeSportsError) ? $result->err->msg : (string)$result->err;
