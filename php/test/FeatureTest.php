...
class FtCtrl extends FreeSportsControl
{
...
class FtClient
{
    ...
    private FreeSportsUtility $utility;

    public function __construct(FreeSportsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeSportsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeSportsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new FreeSportsUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (FreeSportsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeSportsConfig::make_feature($name);
    ...
    private static function buildUrl(FreeSportsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeSportsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeSportsResponse($fetched);
        }
    ...
    private function fail(FreeSportsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeSportsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
