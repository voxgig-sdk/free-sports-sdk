...
class PlayerEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FreeSportsSDK::test($seed, null);
        $seen = iterator_to_array($base->Player(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FreeSportsConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FreeSportsSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Player(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_PLAYER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function player_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["player01", "player02", "player03", "api_key01"] as $k) {
    ...
    $entid_env_raw = getenv("FREESPORTS_TEST_PLAYER_ENTID");
