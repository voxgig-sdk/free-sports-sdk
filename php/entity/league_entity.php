...
class LeagueEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LeagueEntity($this->_client, $opts);
    }

    /**
     * @param League|array $args League data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FreeSportsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return League|array The current League data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of League fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FreeSportsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of League fields).
     */
     ...
     *
     * @param LeagueListMatch|array|null $reqmatch Match filter (any subset
     *   of League fields) as an assoc-array; LeagueListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return League[]|array A list of League items as assoc-arrays at
     *   the SDK boundary; throws FreeSportsError on failure (item-5 convention).
     */
