...
func TestEventEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_EVENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func eventBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"event01", "event02", "event03", "api_key01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("FREESPORTS_TEST_EVENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREESPORTS_TEST_EVENT_ENTID"])
