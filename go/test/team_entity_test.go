...
func TestTeamEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_TEAM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func teamBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"team01", "team02", "team03", "api_key01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("FREESPORTS_TEST_TEAM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREESPORTS_TEST_TEAM_ENTID"])
