...
func TestPlayerEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Player(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Player(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREESPORTS_TEST_PLAYER_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		playerRef01Match := map[string]any{
			"api_key": setup.idmap["api_key01"],
		}

		playerRef01ListResult, err := playerRef01Ent.List(playerRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, playerRef01ListOk := playerRef01ListResult.([]any)
		if !playerRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", playerRef01ListResult)
		}
...
func playerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"player01", "player02", "player03", "api_key01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("FREESPORTS_TEST_PLAYER_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"FREESPORTS_TEST_PLAYER_ENTID": idmap,
		"FREESPORTS_TEST_LIVE":      "FALSE",
		"FREESPORTS_TEST_EXPLAIN":   "FALSE",
		"FREESPORTS_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["FREESPORTS_TEST_PLAYER_ENTID"])
